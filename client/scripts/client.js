...
class LoginScreen {
	...
	static setupBackgroundCanvas() {
		...
		const createFloatingBubble = ()=>{
			return new FloatingBubble(
				20 + Math.random()*10,
				this.backgroundCanvasStuff.floatingBubbleColours[Math.floor(
					Math.random() * this.backgroundCanvasStuff.floatingBubbleColours.length
				)],
				{
					x: this.backgroundCanvasStuff.canvas.width * Math.random(),
					y: this.backgroundCanvasStuff.canvas.height * Math.random()
				},
				{
		...
		for (let i=0; i<100; i+=1) {
			this.backgroundCanvasStuff.floatingBubbles.push( createFloatingBubble() );
		}
